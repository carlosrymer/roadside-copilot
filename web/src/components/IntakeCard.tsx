...
function Field({ label, value }: { label: string; value?: string }) {
  return (
    <div className="flex flex-col">
      ...
      <dd className={value ? 'text-sm text-slate-100' : 'text-sm italic text-slate-600'}>
        {value ?? 'listening…'}
      </dd>
    </div>
  );
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-white">Claim intake</h3>
        {claim.injuriesReported && (
          <span className="rounded-full bg-rose-900/70 px-2 py-0.5 text-[11px] font-medium text-rose-200">
            Injuries reported
          </span>
        )}
      </div>
